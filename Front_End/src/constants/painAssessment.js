export const VERBAL_ABILITY = {
  VERBAL: {value: 'verbal', label: 'Verbal'},
  NON_VERBAL: {value: 'nonVerbal', label: 'Non Verbal'},
  OTHER: {value: 'other', label: 'Other'},
};

export const PAIN_LOCATIONS = [
  {value: 'kneeCap', label: 'Knee Cap'},
  {value: 'aaa', label: 'AAA'},
  {value: 'bbb', label: 'BBB'},
  {value: 'ccc', label: 'CCC'},
];

export const PAIN_QUALITIES = [
  {value: 'sharp', label: 'Sharp'},
  {value: 'dull', label: 'Dull'},
  {value: 'burning', label: 'Burning'},
  {value: 'crushing', label: 'Crushing'},
  {value: 'tearing', label: 'Tearing'},
  {value: 'xxx', label: 'xxx'},
  {value: 'others', label: 'Others'},
];

export const PAIN_FREQUENCY = [
  {value: 'constant', label: 'Constant'},
  {
    value: 'Fluctuate (better and worse)',
    label: ' Fluctuate (better and worse)',
  },
  {value: 'intermittent', label: 'Intermittent'},
  {value: 'Unpredictable', label: 'Unpredictable'},
  {value: 'Intermittent activity based', label: 'Intermittent activity based'},
  {value: 'Recurrent', label: 'Recurrent'},
];

export const PAIN_CAUSING_ACTIVITIES = [
  {value: 'walking', label: 'Walking'},
  {value: 'sleeping', label: 'Sleeping'},
  {value: 'therapy', label: 'Going to the therapy'},
  {value: 'talking', label: 'Talking'},
  {value: 'xxx', label: 'xxx'},
  {value: 'yyy', label: 'yyy'},
  {value: 'others', label: 'Others'},
];
