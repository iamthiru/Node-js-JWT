            ...
            for (x, y, r) in circles:
                if len(iris_radii) == 0 or len(iris_radii) <= 15:
                    ...
                    if ((x_ll <= x <= x_ul) and (y_ll <= y <= y_ul)) and (r_ll <= r <= r_ul):
                        ...
                        r = np.average(iris_radii[-5:])
                        iris_radii.append(r)
                        cv2.circle(im, (x_new, y_new), int(r), (255, 0, 0), 1)
                        # cv2.rectangle(im, (x_ll, y_ll), (x_ul, y_ul), (255, 0, 0), 1)
                        iris_xpoints.append(x_new)
                        iris_ypoints.append(y_new)
                        iris_center = [x_new, y_new]
                Iris_Dilation.append(r)
            ...
            for (x, y, r) in circles:
                if len(pupil_radii) == 0 or len(pupil_radii) < 15:
                    ...
                    if ((x_ll <= x <= x_ul) and (y_ll <= y <= y_ul)) and (r_ll <= r <= r_ul):
                        ...
                        pupil_radii.append(r)
                        cv2.circle(im, (x_new, y_new), int(r), (0, 255, 0), 1)
                        pupil_xpoints.append(x_new)
                        pupil_ypoints.append(y_new)
                        pupil_center = [x_new, y_new]
