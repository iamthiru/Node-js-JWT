...
def Iris_Detection(im, pup_cen):
    ...
    # NIR Videos
        ...
        _, thresh = cv2.threshold(imge, 70, 255, cv2.THRESH_BINARY)
        canny_img = cv2.Canny(thresh, 10, 100)
        circles = cv2.HoughCircles(canny_img, cv2.HOUGH_GRADIENT, 7, 7000, param1=1500, param2=30, minRadius=int(h / 1.9), maxRadius=int(w / 4))
        if circles is not None:
            circles = np.round(circles[0, :]).astype("int")

            for (x, y, r) in circles:
                cv2.circle(im, (pup_cen[0], pup_cen[1]), r, (255, 255, 255), 1)
                radii.append(r)
...
def Pupil_Detection(im, iris_cen):
    ...
    # NIR Videos
    if len(iris_cen) == 0:
        _, thresh = cv2.threshold(imge, 37, 255, cv2.THRESH_BINARY)
...
def Detect_Eye_Points(imge):
    detect = detector(imge, 1)
    if len(detect) != 0:
        shape = predictor(imge, detect[0])
        left_x1 = shape.part(36).x
        left_x2 = shape.part(39).x
        left_y1 = shape.part(37).y
        left_y2 = shape.part(40).y
        lefteye = imge[left_y1 - 5:left_y2 + 10, left_x1:left_x2]
        # print(lefteye.shape)
    return lefteye
...
if len(sys.argv) > 1:
    ...
    ch = 2
    video_type = 'NIR'
    video = cv2.VideoCapture('/Users/pranavdeo/Desktop/NIR Face/4K_Wr_2_HArm/' + filename)
    ...
    if counter % 2 == 0:
        ...
        if ret:
            ...
            for face in faces:
                ...
                rec_face = im[y:y + h, x:x + w]
                # print(rec_face.shape)
                # cv2.imshow('Face', rec_face)
                rec_face = Scale(rec_face, 120)
                eye_frame = Detect_Eye_Points(rec_face)
                ...
                if video_type == 'NIR':
                    Pupil, Pupil_center = Pupil_Detection(eye_frame, emptylist)
                    Iris = Iris_Detection(eye_frame, Pupil_center)
                    cv2.imshow('NIR', Iris)
                    flag = 1

                elif video_type == 'Color':
                    Iris, Iris_center = Iris_Detection(eye_frame, emptylist)
